...
const Doctor = () => {
  ...
  useEffect(() => {
    ...
    const loadReport = async () => {
      ...
      setReportError("");
      try {
        const response = await apiClient.get<DoctorPatientReportResponse>(`/doctor/patients/${selectedPatientId}/report`);
        if (response.error || !response.data) {
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr,1fr]">
        <article className="glass-card rounded-3xl border-border/50 p-5 sm:p-6">
          ...
          <p className="mb-4 text-sm text-muted-foreground">
            Scan patient's QR/code or enter connect code manually.
          </p>
